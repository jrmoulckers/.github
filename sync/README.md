...
| `test/injected-seams.test.mjs` | Inventory of function-valued default parameters in `sync/lib`. A function passed as an injected default is called under the parameter's name, so a grep for its own name reports it as uncalled; the inventory makes reachability readable instead of searchable. |
