...
Since that table was written, `canon-formatting` was added to every member's profile, so every
member also receives `.github/instructions/canon-formatting.instructions.md`. It carries the
formatter obligation previously stated only in `docs/sync.md`, which is synced to nobody — the rule
now arrives in the tree it governs.
