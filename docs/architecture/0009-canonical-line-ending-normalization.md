...
Line endings were unmanaged across the fleet. Seven repositories had **no `.gitattributes` at all** —
`jrm-recipes`, `score-king`, `cartridge`, `docket`, `windows`, `game-library`, and this backbone
repo itself. Six carried the generic `* text=auto eol=lf` stanza: `finance`, `libro`, `engineering`,
`product`, `homelab`, `studio`. Nothing kept the two groups from diverging further, because nothing
owned the rule.
...
The obvious fix — open seven pull requests adding the same three lines — reproduces the problem it
solves. It leaves thirteen independently editable copies of a rule with no owner, which is exactly
the duplicated-normative-text failure [ADR-0003](0003-four-authority-topology.md) exists to prevent,
and the drift would be invisible until the next Windows contributor hit it.

`jrmoulckers/studio` already held a good reference implementation, but it also showed why a
whole-file copy is the wrong transport:
...
Only the first stanza is universal. The second is Studio's, and other members have their own
equivalents — binary patterns, Git LFS filters, `linguist-generated` overrides. A repository's
`.gitattributes` is a genuinely shared file: part canon, part local.
...
5. **The backbone applies its own canon.** `.gitattributes` at the backbone root *is* the canon
   source, which both fixes this repo (one of the seven) and makes the rule self-enforcing: the
...
**Seven hand-written pull requests.** Fastest to land, and the reason the problem exists. Thirteen
unowned copies drift, and the next repository added to the fleet starts without the rule again.

**A whole-file `literal` kind, like `agency.toml`.** Simplest transport, but wrong for a file that is
legitimately co-owned. It would have to either overwrite Studio's tokens rule and every member's LFS
configuration, or be perpetually in drift. `agency.toml` can be a whole-file copy precisely because
no member has a reason to add to it.
