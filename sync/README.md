...
| `test/injected-seams.test.mjs` | Inventory of function-valued default parameters across the whole engine — every `.mjs` under `sync/` and `principles/` that is not a test. A function passed as an injected default is called under the parameter's name, so a grep for its own name reports it as uncalled; the inventory makes reachability readable instead of searchable. The corpus is derived from what makes a file subject to the rule rather than from one directory, and is cross-checked against `git ls-files`, an enumeration that shares none of the walk's logic. |
| `test/instruction-integrity.test.mjs` | Canonical instruction filename/roster parity, deterministic `applyTo` scopes, required non-empty `description` on every canonical instruction, source/materialized ownership, precedence, curated member compatibility, infrastructure routing, local-agent collision safety, immutable reusable-workflow examples, and the hub-local notice on `docs/sync.md` tracking whether `docs/` is a source path. |
